...
function getLogger(categoryName) {
	...
	var levels = ['log', 'debug', 'info', 'warn', 'error', 'trace', 'fatal'];
	var levelIndex = -1;
	if (process.env.LOG_LEVEL) {
		levelIndex = levels.indexOf(process.env.LOG_LEVEL.toLowerCase());
		if(levelIndex>-1){
			logger.warn("set logger min level to "+levels[levelIndex]);
		}
	}
